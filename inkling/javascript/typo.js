/* Slightly modified version of the original typo.js 
 * Modified to fit the Inkling theme elements (the commentform)
 */
...
register_onload(function() {
  if ($('commentform')) {
    $('comment_author').value = getCookie('author');
    $('comment_url').value = getCookie('url');
    
    if ($('comment_url').value != '' || $('comment_email').value != '') {
      Element.show('lbl-url'); Element.show('fld-url'); 
			Element.show('lbl-email'); Element.show('fld-email');
    }
  }
})
